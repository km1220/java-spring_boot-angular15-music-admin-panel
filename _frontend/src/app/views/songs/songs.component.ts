...
export class SongsComponent implements OnInit {
  ...
  constructor(public iconSet: IconSetService, private _songsService: SongsService) {
    iconSet.icons = { ...iconSubset };
  }

  ngOnInit(): void {
    this._songsService.getSongsData().then(res => {
  ...
  handleAddClick() {
    ...
    this._songsService.addSongs(this.new_song)
      .pipe(
        map(event => {
          switch (event.type) {
  ...
  handleEditClick() {
    this._songsService.updateSongs(this.edit_song).then(res => {
      if (res !== undefined) {
  ...
  handleRemoveClick(id: number) {
    this._songsService.removeSongs(id).then(res => {
      console.log('=> res ==========', res)
  ...
  handleOfflineDownloadChange(event: any) {
    ...
    this._songsService.updateOfflineDownloadSongs(data).then(res => {
      console.log('res : ', res);
    });
  }
